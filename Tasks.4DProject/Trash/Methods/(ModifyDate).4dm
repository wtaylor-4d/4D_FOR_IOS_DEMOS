...
If (Length:C16(String:C10($Obj_in.table))>0)
	...
	If ($Obj_in.ID#Null:C1517)
		...
		If ($selection.length>0)
			...
			If (($Obj_in.DueDate#Null:C1517) & ($Obj_in.StartDate#Null:C1517))
				If ($Obj_in.StartDate<$Obj_in.DueDate)
					
					$record.DueDate:=$Obj_in.DueDate
					$record.StartDate:=$Obj_in.StartDate
					...
					End if 
				Else 
					$Obj_out.errors:=New collection:C1472("Start date must not be after Due date")
					
				End if 
				
			Else 
				$Obj_out.errors:=New collection:C1472("No date available")
